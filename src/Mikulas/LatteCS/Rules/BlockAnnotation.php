	...
	protected function run(array $tokens, $content)
	{
		...
		foreach ($tokens as $pos => $token)
		{
			...
			else if ($token->type === $token::TEXT)
			{
				...
				{
					$this->error('Annotation', "Block #$block is not properly annotated", $token->line);
					$expectingComment = FALSE;
				}
			}
			else if ($token->type === $token::COMMENT)
			{
				...
				{
					$this->error('TwoStars', 'Lattedoc does not start with two stars but it should', $token->line);
				}

				$matches = [];
				if (!preg_match_all('~^\s*[*]\s+(?P<params>@param.*?)$~m', $token->text, $matches))
				{
					$this->error('NoParam', "Block #$block is annotated, but no @param was specified.", $token->line);
				}
				else
				{
					foreach ($matches['params'] as $param)
					{
						...
						{
							$this->error('InvalidSyntax', "Invalid annotation '$param', expected '@param type \$name'", $token->line);
						}
						else
						{
							if (strpos($match['name'], '$') === FALSE)
							{
								$this->error('MissingDollar', "Invalid annotation '$param', variable name must start with '\$'", $token->line);
							}
